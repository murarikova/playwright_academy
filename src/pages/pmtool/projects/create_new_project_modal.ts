...
export class CreateNewProjectModal {
  ...
  async triggerNameValidation() {
    ...
    // ! Cisteni pole name (bez vycisteni nam hrozi ze se projekt ulozi pokud bude pole name vyplneno)
    await expect(
      this.nameInput,
      "Assert Name Inout Value is not Filled"
    ).toHaveValue("");
    await this.saveButton.click();
    return this;
  }

  // ? Toto je duplicitni metoda k triggerValidation(), dela to same. Ciel teto metody je ale pomoci pri psani testu se srozumitelnosti PO - když bude tester hledat metodu na zobrazení error hlášky, tak nebude hledat: triggerNameValidation a to může vést k problémům.
