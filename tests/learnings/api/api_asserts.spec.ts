...
test("Response Body Asserts", async ({ request }) => {
  ...
  //* Asserty existujici property (napr property message)
  expect(
    responseBody,
    "Property Timestamp Should Exists in Response Body"
  ).toHaveProperty("timestamp");

  expect(
    responseBody.timestamp,
    "Property Timestamp Should Exists in Response Body"
  ).toBeDefined(); // ? Alternativa k ToHaveProperty - Stejny vysledek
  // * Asserty datoveho typu (message je string)
